...
func GinLoggerWithConfig(config LoggingConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		switch logLevel {
		case "debug":
			logger.Debug("gin request", logAttributes...)
		case "info":
			logger.Info("gin request", logAttributes...)
		case "warn":
			logger.Warn("gin request", logAttributes...)
		case "error":
			logger.Error("gin request", logAttributes...)
		case "skip":
			// Don't log anything
			return
		}
